...
import { ListaEscuelas, noCensados, obtenerCursos, Modificar } from "@/hooks/prot";
...
const Editar = ({ nino }) => {
    ...
    const handleGuardar = async () => {
        ...
        if (
            ...
            const response = await Modificar(
                formData,
                escuelaSeleccionada,
                cursoSeleccionado, nino.extrenal_censo
            );
    ...
    useEffect(() => {
        const obtenerEscuelas = async () => {
            try {
                ...
                // Seleccionar automáticamente la escuela correspondiente al niño
                const escuelaDelNino = listaDeEscuelas.find(escuela => escuela.external_id === nino.escuelaId);
                setEscuelaSeleccionada(escuelaDelNino);
            } catch (error) {
    ...
    useEffect(() => {
        const obtenerIncensados = async () => {
            try {
                ...
                // Seleccionar automáticamente al niño correspondiente
                const ninoDelCombo = listaIncensados.find(incensado => incensado.external_id === nino.id);
                setNinoIncensadaSeleccionada(ninoDelCombo);
            } catch (error) {
    ...
    useEffect(() => {
        const obtenerCurso = async () => {
            try {
                ...
                // Seleccionar automáticamente el curso correspondiente al niño
                const cursoDelNino = listaDeCursos.find(curso => curso.external_id === nino.cursoId);
                setCursoSeleccionado(cursoDelNino);
            } catch (error) {
    ...
    const [formData, setFormData] = useState({
        weight: nino.peso,
        height: nino.talla,
        representative: nino.representante,
        activities: nino.actividades
    });
    ...
    return (
        <div className="container mt-4">
            <h1 className="mb-4">Editar censo</h1>
